                               ...
                               const PCHAR pCertificate, const PCHAR pPrivateKey)
{
    ...
    if (retStatus == STATUS_SUCCESS) {
        ...
        mbedtls_ssl_set_bio(&(pNetworkContext->ssl), &(pNetworkContext->server_fd), mbedtls_net_send, NULL, mbedtls_net_recv_timeout);
        ...
        if ((ret = mbedtls_ssl_config_defaults(&(pNetworkContext->conf), MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM,
            ...
            if (bHasX509Certificate) {
                ...
                if ((ret = mbedtls_x509_crt_parse_file(pNetworkContext->pRootCA, (VOID*) pRootCA)) != 0) {
                ...
                } else if ((ret = mbedtls_x509_crt_parse_file(pNetworkContext->pClientCert, (VOID*) pCertificate)) != 0) {
                    DLOGE("x509 client cert parse err (%d)", ret);
                    retStatus = STATUS_NETWORK_SETUP_ERROR;
                    //} else if ((ret = mbedtls_pk_parse_key(pNetworkContext->pPrivateKey, (VOID*) pPrivateKey, strlen(pPrivateKey) + 1, NULL, 0)) !=
                    // 0) {
                } else if ((ret = mbedtls_pk_parse_keyfile(pNetworkContext->pPrivateKey, (VOID*) pPrivateKey, NULL)) != 0) {
