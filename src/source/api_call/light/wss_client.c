...
INT32 wss_client_wantRead(WssClientContext* pWssCtx)
{
    WSS_CLIENT_ENTER();
...
STATUS wss_client_onReadEvent(WssClientContext* pWssCtx)
{
    WSS_CLIENT_ENTER();
    STATUS retStatus = STATUS_SUCCESS;
    CLIENT_LOCK(pWssCtx);
...
static STATUS wss_client_send(WssClientContext* pWssCtx, struct wslay_event_msg* arg)
{
    WSS_CLIENT_ENTER();
    STATUS retStatus = STATUS_SUCCESS;
    CLIENT_LOCK(pWssCtx);
...
STATUS wss_client_sendText(WssClientContext* pWssCtx, UINT8* buf, UINT32 len)
{
...
STATUS wss_client_sendBinary(WssClientContext* pWssCtx, UINT8* buf, UINT32 len)
{
    struct wslay_event_msg arg;
...
STATUS wss_client_sendPing(WssClientContext* pWssCtx)
{
    struct wslay_event_msg arg;
...
PVOID wss_client_start(WssClientContext* pWssCtx)
{
    WSS_CLIENT_ENTER();
    STATUS retStatus = STATUS_SUCCESS;
    ...
    while (wss_client_wantRead(pWssCtx)) {
        ...
        if (FD_ISSET(nfds, &rfds)) {
            if (wss_client_onReadEvent(pWssCtx)) {
        ...
        if (counter == WSS_CLIENT_PING_PONG_COUNTER) {
            CHK_STATUS(wss_client_sendPing(pWssCtx));
            pWssCtx->pingCounter++;
            counter = 0;
        }
    }
...
VOID wss_client_close(WssClientContext* pWssCtx)
{
    INT32 retStatus = 0;
