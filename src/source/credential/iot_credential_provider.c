...
STATUS iotCurlHandler(PIotCredentialProvider pIotCredentialProvider)
{
    ...
    UINT64 currentTime;
    UINT32 formatLen = 0;
    PCHAR pServiceUrl = NULL;
    PRequestInfo pRequestInfo = NULL;
    CallInfo callInfo;

    CHK(NULL != (pServiceUrl = (PCHAR) MEMCALLOC(MAX_URI_CHAR_LEN + 1, SIZEOF(CHAR))), STATUS_NOT_ENOUGH_MEMORY);
    ...
    formatLen = SNPRINTF(pServiceUrl, MAX_URI_CHAR_LEN, "%s%s%s%c%s%s", CONTROL_PLANE_URI_PREFIX, pIotCredentialProvider->iotGetCredentialEndpoint,
    ...
    CHK_STATUS(createRequestInfo(pServiceUrl, NULL, DEFAULT_AWS_REGION, pIotCredentialProvider->caCertPath, pIotCredentialProvider->certPath,
                                              ...
                                              PAwsCredentialProvider* ppCredentialProvider)
{
    ...
    return createIotCredentialProviderWithTime(iotGetCredentialEndpoint, certPath, privateKeyPath, caCertPath, roleAlias, thingName, getCurrentTimeFn,
                                               customData, NULL, ppCredentialProvider);
}
