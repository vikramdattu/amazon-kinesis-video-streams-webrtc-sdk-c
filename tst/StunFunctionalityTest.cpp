...
TEST_F(StunFunctionalityTest, basicValidParseTest)
{
    ...
    EXPECT_EQ(STATUS_SUCCESS,
              stun_deserializePacket(bindingRequestBytes, SIZEOF(bindingRequestBytes), (PBYTE) TEST_STUN_PASSWORD,
    ...
    //
    EXPECT_EQ(STATUS_SUCCESS,
              stun_deserializePacket(bindingSuccessResponseXorMappedAddressBytes1, SIZEOF(bindingSuccessResponseXorMappedAddressBytes1),
    ...
    //
    EXPECT_EQ(STATUS_SUCCESS,
              stun_deserializePacket(bindingSuccessResponseXorMappedAddressBytes2, SIZEOF(bindingSuccessResponseXorMappedAddressBytes2),
                                    (PBYTE) TEST_STUN_PASSWORD, (UINT32) STRLEN(TEST_STUN_PASSWORD) * SIZEOF(CHAR), &pStunPacket));
    EXPECT_EQ(pStunPacket->header.magicCookie, STUN_HEADER_MAGIC_COOKIE);
    ...
    //
    EXPECT_EQ(STATUS_SUCCESS,
              stun_deserializePacket(bindingRequestUsernameBytes, SIZEOF(bindingRequestUsernameBytes), (PBYTE) TEST_STUN_PASSWORD,
                                    (UINT32) STRLEN(TEST_STUN_PASSWORD) * SIZEOF(CHAR), &pStunPacket));
    EXPECT_EQ(pStunPacket->header.magicCookie, STUN_HEADER_MAGIC_COOKIE);
    ...
    EXPECT_EQ(STATUS_SUCCESS, stun_attribute_getByType(pStunPacket, STUN_ATTRIBUTE_TYPE_FINGERPRINT, &pAttribute));
    EXPECT_TRUE(NULL != pAttribute);

    EXPECT_EQ(STATUS_SUCCESS, stun_attribute_getByType(pStunPacket, STUN_ATTRIBUTE_TYPE_MESSAGE_INTEGRITY, &pAttribute));
    EXPECT_TRUE(NULL != pAttribute);

    EXPECT_EQ(STATUS_SUCCESS, stun_attribute_getByType(pStunPacket, STUN_ATTRIBUTE_TYPE_PRIORITY, &pAttribute));
    EXPECT_TRUE(NULL != pAttribute);

    EXPECT_EQ(STATUS_SUCCESS, stun_attribute_getByType(pStunPacket, STUN_ATTRIBUTE_TYPE_USERNAME, &pAttribute));
    EXPECT_TRUE(NULL != pAttribute);

    pAttribute = NULL;
    EXPECT_EQ(STATUS_SUCCESS, stun_attribute_getByType(pStunPacket, STUN_ATTRIBUTE_TYPE_XOR_MAPPED_ADDRESS, &pAttribute));
    EXPECT_TRUE(NULL == pAttribute);
    ...
    //
    EXPECT_EQ(STATUS_SUCCESS,
              stun_deserializePacket(bindingSuccessResponseXorMappedAddressBytes3, SIZEOF(bindingSuccessResponseXorMappedAddressBytes3), NULL, 0,
                                    &pStunPacket));
    EXPECT_EQ(pStunPacket->header.magicCookie, STUN_HEADER_MAGIC_COOKIE);
    EXPECT_EQ(pStunPacket->header.messageLength, 12);
    EXPECT_EQ(pStunPacket->header.stunMessageType, STUN_PACKET_TYPE_BINDING_RESPONSE_SUCCESS);
    ...
    //
    EXPECT_EQ(STATUS_SUCCESS,
              stun_deserializePacket(bindingSuccessResponseXorMappedAddressBytes4, SIZEOF(bindingSuccessResponseXorMappedAddressBytes4), NULL, 0,
                                    &pStunPacket));
    EXPECT_EQ(pStunPacket->header.magicCookie, STUN_HEADER_MAGIC_COOKIE);
    EXPECT_EQ(pStunPacket->header.messageLength, 12);
    EXPECT_EQ(pStunPacket->header.stunMessageType, STUN_PACKET_TYPE_BINDING_RESPONSE_SUCCESS);
...
TEST_F(StunFunctionalityTest, roundtripFidelityTest)
{
    ...
    // Serialize it
    EXPECT_EQ(STATUS_SUCCESS,
              stun_serializePacket(pStunPacket, (PBYTE) TEST_STUN_PASSWORD, STRLEN(TEST_STUN_PASSWORD) * SIZEOF(CHAR), TRUE, TRUE, NULL, &size));
    EXPECT_TRUE(NULL != (pBuffer = (PBYTE) MEMALLOC(size)));
    EXPECT_EQ(STATUS_SUCCESS,
              stun_serializePacket(pStunPacket, (PBYTE) TEST_STUN_PASSWORD, STRLEN(TEST_STUN_PASSWORD) * SIZEOF(CHAR), TRUE, TRUE, pBuffer, &size));

    // De-serialize it back again
    EXPECT_EQ(
        STATUS_SUCCESS,
        stun_deserializePacket(pBuffer, size, (PBYTE) TEST_STUN_PASSWORD, (UINT32) STRLEN(TEST_STUN_PASSWORD) * SIZEOF(CHAR), &pSerializedStunPacket));
    ...
    EXPECT_EQ(STATUS_SUCCESS, stun_freePacket(&pStunPacket));
    EXPECT_EQ(STATUS_SUCCESS, stun_freePacket(&pSerializedStunPacket));
...
TEST_F(StunFunctionalityTest, appendAddressAfterParseTest)
{
    ...
    EXPECT_EQ(STATUS_NOT_ENOUGH_MEMORY, stun_attribute_appendAddress(pStunPacket, STUN_ATTRIBUTE_TYPE_MAPPED_ADDRESS, &address));
    EXPECT_EQ(STATUS_SUCCESS, stun_freePacket(&pStunPacket));
    ...
    //
    EXPECT_EQ(STATUS_SUCCESS,
              stun_deserializePacket(bindingSuccessResponseXorMappedAddressBytes, SIZEOF(bindingSuccessResponseXorMappedAddressBytes),
                                    (PBYTE) TEST_STUN_PASSWORD, (UINT32) STRLEN(TEST_STUN_PASSWORD) * SIZEOF(CHAR), &pStunPacket));
    ...
    EXPECT_EQ(STATUS_STUN_ATTRIBUTES_AFTER_FINGERPRINT_MESSAGE_INTEGRITY,
              stun_attribute_appendAddress(pStunPacket, STUN_ATTRIBUTE_TYPE_MAPPED_ADDRESS, &address));
    EXPECT_EQ(STATUS_SUCCESS, stun_freePacket(&pStunPacket));
}
...
TEST_F(StunFunctionalityTest, appendUsernameAfterParseTest)
{
    ...
    // Attempt to add to it should be out of memory
    EXPECT_EQ(STATUS_NOT_ENOUGH_MEMORY, stun_attribute_appendUsername(pStunPacket, userName0));
    EXPECT_EQ(STATUS_NOT_ENOUGH_MEMORY, stun_attribute_appendUsername(pStunPacket, userName1));
    EXPECT_EQ(STATUS_NOT_ENOUGH_MEMORY, stun_attribute_appendUsername(pStunPacket, userName2));
    EXPECT_EQ(STATUS_NOT_ENOUGH_MEMORY, stun_attribute_appendUsername(pStunPacket, userName3));
    EXPECT_EQ(STATUS_SUCCESS, stun_freePacket(&pStunPacket));
    ...
    // Attempt to add to it should fail as we are adding after integrity/fingerprint
    EXPECT_EQ(STATUS_STUN_ATTRIBUTES_AFTER_FINGERPRINT_MESSAGE_INTEGRITY, stun_attribute_appendUsername(pStunPacket, userName0));
    EXPECT_EQ(STATUS_STUN_ATTRIBUTES_AFTER_FINGERPRINT_MESSAGE_INTEGRITY, stun_attribute_appendUsername(pStunPacket, userName1));
    EXPECT_EQ(STATUS_STUN_ATTRIBUTES_AFTER_FINGERPRINT_MESSAGE_INTEGRITY, stun_attribute_appendUsername(pStunPacket, userName2));
    EXPECT_EQ(STATUS_STUN_ATTRIBUTES_AFTER_FINGERPRINT_MESSAGE_INTEGRITY, stun_attribute_appendUsername(pStunPacket, userName3));
    EXPECT_EQ(STATUS_SUCCESS, stun_freePacket(&pStunPacket));
}
...
TEST_F(StunFunctionalityTest, appendAddressAttributeMaxCountTest)
{
    ...
    for (i = 0; i <= STUN_ATTRIBUTE_MAX_COUNT; i++) {
        EXPECT_EQ(STATUS_SUCCESS, stun_attribute_appendAddress(pStunPacket, STUN_ATTRIBUTE_TYPE_MAPPED_ADDRESS, &address));
    }

    // Should fail with one more
    EXPECT_EQ(STATUS_STUN_MAX_ATTRIBUTE_COUNT, stun_attribute_appendAddress(pStunPacket, STUN_ATTRIBUTE_TYPE_MAPPED_ADDRESS, &address));
...
TEST_F(StunFunctionalityTest, appendUsernameAttributeMaxCountTest)
{
    ...
    for (i = 0; i <= STUN_ATTRIBUTE_MAX_COUNT; i++) {
        EXPECT_EQ(STATUS_SUCCESS, stun_attribute_appendUsername(pStunPacket, userName));
    }

    // Should fail with one more
    EXPECT_EQ(STATUS_STUN_MAX_ATTRIBUTE_COUNT, stun_attribute_appendUsername(pStunPacket, userName));
...
TEST_F(StunFunctionalityTest, appendUsernameAttributeMaxAllocationTest)
{
    ...
    EXPECT_EQ(STATUS_NOT_ENOUGH_MEMORY, stun_attribute_appendUsername(pStunPacket, longUserName));
...
TEST_F(StunFunctionalityTest, attributeDetectionTest)
{
    ...
    EXPECT_EQ(STATUS_SUCCESS, stun_attribute_getByType(pStunPacket, STUN_ATTRIBUTE_TYPE_USERNAME, &pUsernameAttribute));
...
TEST_F(StunFunctionalityTest, roundtripAfterCreateAddFidelityTest)
{
    ...
    EXPECT_EQ(STATUS_SUCCESS, stun_attribute_appendPriority(pStunPacket, 10));
    EXPECT_EQ(STATUS_SUCCESS, stun_attribute_appendFlag(pStunPacket, STUN_ATTRIBUTE_TYPE_USE_CANDIDATE));
    EXPECT_EQ(STATUS_SUCCESS, stun_attribute_appendAddress(pStunPacket, STUN_ATTRIBUTE_TYPE_REFLECTED_FROM, &address));
    EXPECT_EQ(STATUS_SUCCESS, stun_attribute_appendUsername(pStunPacket, (PCHAR) "abc"));
...
TEST_F(StunFunctionalityTest, serializeStunRequestWithNoAttribute)
{
    ...
    EXPECT_EQ(STATUS_SUCCESS, stun_createPacket(STUN_PACKET_TYPE_BINDING_REQUEST, transactionId, &pStunPacket));
    EXPECT_EQ(STATUS_SUCCESS, stun_serializePacket(pStunPacket, NULL, 0, FALSE, FALSE, NULL, &actualPacketSize));
    EXPECT_TRUE(actualPacketSize < stunPacketBufferSize);
    EXPECT_EQ(STATUS_SUCCESS, stun_serializePacket(pStunPacket, NULL, 0, FALSE, FALSE, stunPacketBuffer, &actualPacketSize));
...
TEST_F(StunFunctionalityTest, serializeDeserializeStunControlAttribute)
{
    ...
    EXPECT_EQ(STATUS_SUCCESS, stun_createPacket(STUN_PACKET_TYPE_BINDING_REQUEST, transactionId, &pStunPacket));
    EXPECT_EQ(STATUS_SUCCESS, stun_attribute_appendIceControlMode(pStunPacket, STUN_ATTRIBUTE_TYPE_ICE_CONTROLLED, magicValue));
    EXPECT_EQ(STATUS_SUCCESS, stun_serializePacket(pStunPacket, NULL, 0, FALSE, FALSE, NULL, &actualPacketSize));
    EXPECT_TRUE(actualPacketSize < stunPacketBufferSize);
    EXPECT_EQ(STATUS_SUCCESS, stun_serializePacket(pStunPacket, NULL, 0, FALSE, FALSE, stunPacketBuffer, &actualPacketSize));
    EXPECT_TRUE(IS_STUN_PACKET(stunPacketBuffer));

    EXPECT_EQ(STATUS_SUCCESS, stun_deserializePacket(stunPacketBuffer, actualPacketSize, NULL, 0, &pDeserializedPacket));
    EXPECT_EQ(STATUS_SUCCESS, stun_attribute_getByType(pDeserializedPacket, STUN_ATTRIBUTE_TYPE_ICE_CONTROLLED, &pStunAttributeHeader));
    ...
    EXPECT_EQ(STATUS_SUCCESS, stun_freePacket(&pStunPacket));
    EXPECT_EQ(STATUS_SUCCESS, stun_freePacket(&pDeserializedPacket));
}
...
TEST_F(StunFunctionalityTest, serializeDeserializeXORAddress)
{
    ...
    //
    EXPECT_EQ(STATUS_SUCCESS, stun_createPacket(STUN_PACKET_TYPE_BINDING_REQUEST, transactionId, &pStunPacket));
    EXPECT_EQ(STATUS_SUCCESS, stun_attribute_appendAddress(pStunPacket, STUN_ATTRIBUTE_TYPE_XOR_MAPPED_ADDRESS, &address));
...
TEST_F(StunFunctionalityTest, deserializeStunErrorCode)
{
    ...
    EXPECT_EQ(STATUS_SUCCESS, stun_deserializePacket(stunErrorBuffer, ARRAY_SIZE(stunErrorBuffer), turnKey, KVS_MD5_DIGEST_LENGTH, &pStunPacket));

    EXPECT_EQ(STATUS_SUCCESS, stun_attribute_getByType(pStunPacket, STUN_ATTRIBUTE_TYPE_ERROR_CODE, &pStunAttr));
