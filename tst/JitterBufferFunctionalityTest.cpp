...
TEST_F(JitterBufferFunctionalityTest, continousPacketsComeInOrder)
{
    ...
    for (i = 0; i < pktCount; i++) {
        EXPECT_EQ(STATUS_SUCCESS, jitter_buffer_push(mJitterBuffer, mPRtpPackets[i], nullptr));
        switch (i) {
            case 0:
...
TEST_F(JitterBufferFunctionalityTest, fillDataGiveExpectedData)
{
    ...
    EXPECT_EQ(STATUS_SUCCESS, jitter_buffer_fillFrameData(mJitterBuffer, buffer, 2, &filledSize, 0, 1));
...
TEST_F(JitterBufferFunctionalityTest, fillDataReturnErrorWithImcompleteFrame)
{
    ...
    EXPECT_EQ(STATUS_NULL_ARG, jitter_buffer_fillFrameData(mJitterBuffer, buffer, 2, &filledSize, 0, 1));
...
TEST_F(JitterBufferFunctionalityTest, fillDataReturnErrorWithNotEnoughOutputBuffer)
{
    ...
    EXPECT_EQ(STATUS_BUFFER_TOO_SMALL, jitter_buffer_fillFrameData(mJitterBuffer, buffer, 1, &filledSize, 0, 1));
...
TEST_F(JitterBufferFunctionalityTest, dropDataGivenSmallStartAndLargeEnd)
{
    ...
    EXPECT_EQ(STATUS_SUCCESS, jitter_buffer_dropBufferData(mJitterBuffer, 1, 2, 100));
...
TEST_F(JitterBufferFunctionalityTest, dropDataGivenLargeStartAndSmallEnd)
{
    ...
    EXPECT_EQ(STATUS_SUCCESS, jitter_buffer_dropBufferData(mJitterBuffer, 10, 2, 100));
