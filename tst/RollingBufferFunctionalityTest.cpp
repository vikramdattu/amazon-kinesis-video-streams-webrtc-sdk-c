...
TEST_F(RollingBufferFunctionalityTest, appendDataToBufferAndVerify)
{
    ...
    UINT64 index;
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_create(2, RollingBufferFunctionalityTestFreeBufferFunc, &pRollingBuffer));
    EXPECT_EQ(0, pRollingBuffer->headIndex);
    EXPECT_EQ(0, pRollingBuffer->tailIndex);
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_appendData(pRollingBuffer, first, &index));
    EXPECT_EQ(1, pRollingBuffer->headIndex);
    EXPECT_EQ(0, pRollingBuffer->tailIndex);
    EXPECT_EQ(first, pRollingBuffer->dataBuffer[0]);
    EXPECT_EQ(0, index);
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_appendData(pRollingBuffer, second, &index));
    EXPECT_EQ(2, pRollingBuffer->headIndex);
    EXPECT_EQ(0, pRollingBuffer->tailIndex);
    EXPECT_EQ(first, pRollingBuffer->dataBuffer[0]);
    EXPECT_EQ(second, pRollingBuffer->dataBuffer[1]);
    EXPECT_EQ(1, index);
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_appendData(pRollingBuffer, third, &index));
    EXPECT_EQ(3, pRollingBuffer->headIndex);
    EXPECT_EQ(1, pRollingBuffer->tailIndex);
    EXPECT_EQ(third, pRollingBuffer->dataBuffer[0]);
    EXPECT_EQ(second, pRollingBuffer->dataBuffer[1]);
    EXPECT_EQ(2, index);
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_appendData(pRollingBuffer, fourth, &index));
    EXPECT_EQ(4, pRollingBuffer->headIndex);
    EXPECT_EQ(2, pRollingBuffer->tailIndex);
    EXPECT_EQ(third, pRollingBuffer->dataBuffer[0]);
    EXPECT_EQ(fourth, pRollingBuffer->dataBuffer[1]);
    EXPECT_EQ(3, index);
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_free(&pRollingBuffer));
}
...
TEST_F(RollingBufferFunctionalityTest, insertDataToBufferAndVerify)
{
    ...
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_create(10, RollingBufferFunctionalityTestFreeBufferFunc, &pRollingBuffer));
    pRollingBuffer->headIndex = 3;
    pRollingBuffer->tailIndex = 1;
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_insertData(pRollingBuffer, 1, first));
    EXPECT_EQ(3, pRollingBuffer->headIndex);
    EXPECT_EQ(1, pRollingBuffer->tailIndex);
    EXPECT_EQ(first, pRollingBuffer->dataBuffer[1]);
    EXPECT_EQ(STATUS_ROLLING_BUFFER_NOT_IN_RANGE, rolling_buffer_insertData(pRollingBuffer, 0, second));
    EXPECT_EQ(3, pRollingBuffer->headIndex);
    EXPECT_EQ(1, pRollingBuffer->tailIndex);
    EXPECT_EQ(NULL, pRollingBuffer->dataBuffer[0]);
    EXPECT_EQ(STATUS_ROLLING_BUFFER_NOT_IN_RANGE, rolling_buffer_insertData(pRollingBuffer, 3, third));
    EXPECT_EQ(3, pRollingBuffer->headIndex);
    EXPECT_EQ(1, pRollingBuffer->tailIndex);
    EXPECT_EQ(NULL, pRollingBuffer->dataBuffer[3]);
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_insertData(pRollingBuffer, 2, fourth));
    EXPECT_EQ(3, pRollingBuffer->headIndex);
    EXPECT_EQ(1, pRollingBuffer->tailIndex);
    EXPECT_EQ(fourth, pRollingBuffer->dataBuffer[2]);
    EXPECT_EQ(STATUS_ROLLING_BUFFER_NOT_IN_RANGE, rolling_buffer_insertData(pRollingBuffer, 5, fifth));
    EXPECT_EQ(3, pRollingBuffer->headIndex);
    EXPECT_EQ(1, pRollingBuffer->tailIndex);
...
TEST_F(RollingBufferFunctionalityTest, extractDataFromBufferAndInsertBack)
{
    ...
    UINT64 data;
    UINT64 index;
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_create(3, RollingBufferFunctionalityTestFreeBufferFunc, &pRollingBuffer));
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_appendData(pRollingBuffer, first, &index));
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_appendData(pRollingBuffer, second, &index));
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_appendData(pRollingBuffer, third, &index));
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_appendData(pRollingBuffer, fourth, &index));
    ...
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_extractData(pRollingBuffer, 2, &data));
    EXPECT_EQ(third, data);
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_extractData(pRollingBuffer, 2, &data));
    EXPECT_EQ(NULL, data);
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_insertData(pRollingBuffer, 2, third));
    EXPECT_EQ(third, pRollingBuffer->dataBuffer[2]);
    EXPECT_EQ(STATUS_SUCCESS, rolling_buffer_insertData(pRollingBuffer, 3, third));
    EXPECT_EQ(third, pRollingBuffer->dataBuffer[0]);
